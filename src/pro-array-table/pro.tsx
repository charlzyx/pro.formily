...
const ArrayTableProInside: ReactFC<ProArrayTableProps> = observer((props) => {
  ...
  const _footer = !showFooter ? null : (
    <Flex between marginTop={`${6}px`}>
      <Flex start hidden={!props.footer && rowSelection?.showPro !== "bottom"}>
        {props.footer ? (
  ...
  const body = useSortableBody(dataSource, (from, to) => field.move(from, to), {
...
ProArrayTable.displayName = "ArrayTablePro";
